...
%% *CHECK INPUT*
if nargin < 2
    output_format = 'SV';                                                          % format of outputed images: can be 'FP', 'DV' or 'SV'
end
...
parms.turbo_factor = unique(parms.tags(:,40));                                     % turbo factor - 0 = off, 1 = ??
...
if (read_size ~= nimg*nline*stride)
    fprintf('Expected %d int.  Found %d int\n',nimg*nline*stride,read_size);       % error catching  
    if (read_size > nimg*nline*stride)
